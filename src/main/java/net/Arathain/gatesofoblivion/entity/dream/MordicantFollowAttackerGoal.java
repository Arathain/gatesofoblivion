...
public class MordicantFollowAttackerGoal<T extends ZombieEntity & BoundEntity> extends Goal{
    ...
    public MordicantFollowAttackerGoal(T tameable, double speed, float minDistance, float maxDistance, boolean leavesAllowed) {
    ...
    public boolean canStart() {
        ...
        this.attacker = this.tameable.getTarget();
        LivingEntity attacker = this.attacker;
        if (this.attacker != null) {
            ...
            else if (this.tameable.squaredDistanceTo(attacker) < (double) (this.minDistance * this.minDistance)) {
                return false;
            } else {
    ...
    public boolean shouldContinue() {
        if (this.navigation.isIdle()) {
            ...
            return this.tameable.squaredDistanceTo(this.attacker) > (double) (this.maxDistance * this.maxDistance);
        }
    }
    ...
    public void tick() {
        this.tameable.getLookControl().lookAt(this.attacker, 10.0F, (float) this.tameable.getLookPitchSpeed());
        if (--this.updateCountdownTicks <= 0) {
            this.updateCountdownTicks = 10;

            if (this.tameable.squaredDistanceTo(this.attacker) >= 500D) {
                this.tryTeleport();
            } else {
                this.navigation.startMovingTo(this.attacker, this.speed);
            }
    ...
    private void tryTeleport() {
        BlockPos blockPos = this.attacker.getBlockPos();
    ...
    private boolean tryTeleportTo(int x, int y, int z) {
        if (Math.abs((double) x - this.attacker.getX()) < 2.0D && Math.abs((double) z - this.attacker.getZ()) < 2.0D) {
            return false;
